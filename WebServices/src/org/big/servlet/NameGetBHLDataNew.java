 ...
 */
public class NameGetBHLDataNew extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public NameGetBHLDataNew() {
        super();
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			Gson gson = new Gson();
			List<Map<String, Object>> nameSearchDataList = new ArrayList<Map<String, Object>>();
			if(job.getJSONObject("Result").getJSONArray("Titles").size() != 0){
				nameSearchDataList = gson.fromJson(job.getJSONObject("Result").getJSONArray("Titles").toString(),
						new TypeToken<List<Map<String, Object>>>() {
				}.getType());
			}
			request.getSession().setAttribute("nameSearchDataList", nameSearchDataList);
			CommonsUtil.requestDispatcher("/resultData?flag=4&num=1", request, response);
			
			
		} catch (UnparsableException e) {
			// TODO Auto-generated catch block
			//e.printStackTrace();
			WebUtil.alert(request, response, "您输入的内容不能解析", "/index.jsp", false);
		}
